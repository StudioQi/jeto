...
A Vagrant instance can be a single machine or contain multiple machines.
...
* Permissions control: Administrators can give permissions at the project level.
...
Vagrant instances can be deployed on Virtualbox, but you might want to deploy your instance on Amazon for example.
...
Your *host* can have two fields : the *provider name* and *parameters*.
Depending on your workflow, you might want to keep some variables at the project level ( Amazon Access Key, VMWare templte name, etc.).
...
Most of the time we advise against it, but in some case you might not have the choice like for example :
...
It can be very useful if you are deploying to a DEV environment or testing new providers.
